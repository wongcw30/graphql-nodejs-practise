...
class Feed extends Component {
	...
	componentDidMount() {
		...
		fetch('http://localhost:8811/graphql', {
			method: 'POST',
			headers: {
	...
	finishEditHandler = (postData) => {
		...
		fetch('http://localhost:8811/post-image', {
			method: 'PUT',
			headers: {
				Authorization: 'Bearer ' + this.props.token
			},
			...
			.then((fileResData) => {
				...
				if (this.state.editPost) {
					graphqlQuery = {
						query: `
						mutation updateCurrentPost( $id: ID!, $title: String!, $content: String!, $imageUrl: String! ) {
							updatePost( id: $id
							postInput: {
								title: $title
								content: $content
				...
				return fetch('http://localhost:8811/graphql', {
					method: 'POST',
					body: JSON.stringify(graphqlQuery),
					headers: {
						Authorization: 'Bearer ' + this.props.token,
